...
public class Configuration {
    ...
    public final List<Integer> stack;
    ...
    public String[] getStackWords() {
        String[] words = new String[getStackSize()];
        for (int stackIndex = 0; stackIndex < getStackSize(); stackIndex++) {
            words[stackIndex] = getWord(stack.get(stackIndex));
        }
        return words;
    }

    public String[] getStackPOSTags() {
        String[] partsOfSpeech = new String[getStackSize()];
        for (int stackIndex = 0; stackIndex < getStackSize(); stackIndex++) {
            partsOfSpeech[stackIndex] = getPOS(stack.get(stackIndex));
        }
        return partsOfSpeech;
    }

    public String[] getBufferWords() {
        String[] words = new String[getBufferSize()];
        for (int bufferIndex = 0; bufferIndex < getBufferSize(); bufferIndex++) {
            words[bufferIndex] = getWord(buffer.get(bufferIndex));
        }
        return words;
    }

    public String[] getBufferPOSTags() {
        String[] partsOfSpeech = new String[getBufferSize()];
        for (int bufferIndex = 0; bufferIndex < getBufferSize(); bufferIndex++) {
            partsOfSpeech[bufferIndex] = getPOS(buffer.get(bufferIndex));
        }
        return partsOfSpeech;
    }

    public List<String> getDependencyArcs() {
        LinkedList<String> arcs = new LinkedList<>();
        for (int i = 0; i <= tree.n; i++) {
            String dependant = getWord(i);
            String head = getWord(getHead(i));
            String label = getLabel(i);
//            String arc = word + " " + head + " " + label;
            String arc = label + "(" + head + ", " + dependant + ")";
            if (head != "-NULL-" && label != "-UNKNOWN-") {
                arcs.add(arc);
            }
        }
        return arcs;
    ...
    public List<String> getPOSDependencyArcs() {
        LinkedList<String> arcs = new LinkedList<>();
        for (int i = 0; i <= tree.n; i++) {
            String dependantPOS = getPOS(i);
            String headPOS = getPOS(getHead(i));
            String label = getLabel(i);
//          String arc = tag1 + " " + tag2 + " " + label;
            String arc = label + "(" + headPOS + ", " + dependantPOS + ")";
            if (headPOS != "-NULL-" && label != "-UNKNOWN-") {
                arcs.add(arc);
            }
        }
        return arcs;
